...
function OrdersTable() {
    ...
    useEffect(() => {
        ...
        const table = $(tableRef.current).DataTable({
            pageLength: 6, // số dòng của một trang
            lengthChange: false, // lựa chọn cho phép thay đổi số dòng mỗi trang
            searching: false, //tắt thanh tìm kiếm
            language: { info: "_TOTAL_ results" }, // chỉnh hiển thị tổng số là results
            columnDefs: [
                { width: "150px", targets: 5 } // Định nghĩa độ rộng cột Status
            ]
        });
        return () => table.destroy(); // khi unmout hoặc arr thay đổi thì sẽ hủy bảng tránh tạo lại trùng lặp
    }, [arr]);
